...
- 服务身份由“协议 + 优先级”唯一确定；使用相同身份重新注册时，新实例会替换旧实例。
- 旧弱引用实例在新服务注册后释放时，不会移除新服务；强引用注册同样会替换旧弱引用服务的生命周期。
- 同一实例可以注册多个协议；实例释放后，各协议服务会独立清理。
- 手动注册支持并发调用。同一服务身份的并发注册会原子提交，最后完成内部注册提交的实例成为当前服务。
